...
"""注意！以下几行代码仅用于第一问第二种情况！"""
# 每年各种农作物产量应小于等于预期销量
for i in range(1, 42):
    for t in range(1, 8):
        model += pulp.lpSum(x[(i, j, k, t)] * crop_yield.get((countryside_planting_crops['作物名称'][i-1], countryside_existing_areas['地块类型'][j-1], which_season[k]), 0) for j in range(1, 55) for k in range(1, 3)) <= list(expected_production.expected_production.values())[i - 1] * 2
