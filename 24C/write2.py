...
#         for j in range(1, 55):
#             for k in range(1, 3):
#                 if x[(i, j, k, t)].varValue < 0:
#                     print(f'x({i, j, k, t}): ', x[(i, j, k, t)].varValue)


# "E:\2024全国大学生数学建模\C题\附件3\result1_1.xlsx"
path = r'E:\2024全国大学生数学建模\C题\附件3'
result1_2_path = os.path.join(path, 'result1_2.xlsx')
result1_2_2024 = pd.read_excel(result1_2_path, sheet_name=0)

result1_2_2024 = result1_2_2024.drop(range(0, 54)).reset_index()

print(result1_2_2024)

# for i in range(1, 42):
#     for j in range(1, 55):
#         result1_2_2024[countryside_planting_crops['作物名称'][i - 1]][j - 1] = x[(i, j, 1, 7)].varValue

for i in range(1, 42):
    for j in range(1, 29):
        result1_2_2024[countryside_planting_crops['作物名称'][i - 1]][j - 1] = x[(i, j+26, 2, 7)].varValue

result1_2_2024.to_excel('result1_2_2024.xlsx')
