...
print('central_warehouses: ', central_warehouses)
regional_warehouses = df_wh.loc[df_wh['Type'] == 'RDC']['Name'].values.tolist()
print('regional_warehouses: ', regional_warehouses)
...
print('warehouse_proc_upper_limits: ', warehouse_proc_upper_limits)
...
print('warehouse_proc_costs: ', warehouse_proc_costs)
...
print('order_demands: ', order_demands)
# print(order_demands["an-shan-shi"])
# print(order_demands["an-shan-shi"]["dm"])
...
temp_own = pulp.LpVariable.dicts("temp_own", [(rw, s) for rw in regional_warehouses for s in stores], lowBound=0, upBound=1, cat='Binary')


handling_cost = pulp.lpSum(
    # [warehouse_proc_costs[cw] * order_demands[s]['im'] * (u_im[cw][s] - 0.5*temp_own[rw][s]) for s in stores for cw in regional_warehouses] +
    # [warehouse_proc_costs[cw] * order_demands[s]['dm'] * (u_im[cw][s] - 0.5*temp_own[rw][s]) for s in stores for cw in regional_warehouses] +
    [((warehouse_proc_costs[rw] * order_demands[s]['dm'] * (v_dm[(rw, s)] - 0.5*temp_own[(rw, s)])) for s in stores) for rw in regional_warehouses] +
    [((warehouse_proc_costs[rw] * order_demands[s]['im'] * (v_im[(rw, s)] - 0.5*temp_own[(rw, s)])) for s in stores) for rw in regional_warehouses]
)
...
# temp_own[rw, s] = theta[rw] * v_im[rw, s]
for rw in regional_warehouses:
    for s in stores:
        model += temp_own[rw, s] <= theta_R[rw]
        model += temp_own[rw, s] <= v_im[rw, s]
        model += temp_own[rw, s] >= theta_R[rw] + v_im[rw, s] - 1

for rw in regional_warehouses:
    for s in stores:
        model += temp_own[rw, s] <= theta_R[rw]
        model += temp_own[rw, s] <= v_dm[rw, s]
        model += temp_own[rw, s] >= theta_R[rw] + v_dm[rw, s] - 1

for rw in regional_warehouses:
    for s in stores:
        model += v_dm[rw, s] <= w_R[rw]
        model += v_im[rw, s] <= w_R[rw]

for rw in regional_warehouses:
    model += w_R[rw] <= pulp.lpSum([v_im[rw, s] for s in stores])
    model += w_R[rw] <= pulp.lpSum([v_dm[rw, s] for s in stores])
...
for cw in central_warehouses:
    for s in stores:
        print(f'u_dm[{cw}, {s}]: ', u_dm[cw, s].value())
print("--------------------------------------------------------------------------------------")
for rw in regional_warehouses:
    for s in stores:
        print(f'v_dm[{rw}, {s}]: ', v_dm[rw, s].value(), '      ', f'v_im[{rw}, {s}]: ', v_im[rw, s].value())
