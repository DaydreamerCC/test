...
pygame.display.set_caption("模拟抓猪 作者：Chen Cong")
        ...
        if ((self.position[0] + 1 < self.field_height and field[self.position[0] + 1][self.position[1]] == 0) or \
            ...
            if random.randint(0, 1):
                ...
                if self.position[1] == self.target.position[1]:  # 如果随机到横走，判断是否已经在同一列，如果在就没必要横走，直接竖走
                    ...
                    if self.position[0] - self.target.position[0] > 0 and field[self.position[0] - 1][
                        self.position[1]] == 0:
                        self.position[0] -= 1  # 更改坐标

                    elif self.position[0] - self.target.position[0] < 0 and field[self.position[0] + 1][
                        self.position[1]] == 0:
                        self.position[0] += 1
                    # 竖向也没得走，随机走一步
                    else:
                ...
                elif self.position[1] - self.target.position[1] > 0 and field[self.position[0]][
                    self.position[1] - 1] == 0:
                    self.position[1] -= 1
            ...
            # 如果随机到竖走，以下逻辑全部同上，仅修改方向
                ...
                if self.position[0] == self.target.position[0]:  # 如果随机到竖走，判断是否已经在同一行，如果在就没必要竖走，直接横走
                    ...
                    if self.position[1] - self.target.position[1] > 0 and field[self.position[0]][
                        self.position[1] - 1] == 0:
                        self.position[1] -= 1
...
class Pig:
    ...
    def capture(self):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if 0 <= self.position[0] + i < field_height and 0 <= self.position[1] + j < field_width and \
                        field[self.position[0] + i][self.position[1] + j] == 1:
                    human_num += 1
        # 大于等于三个人就认为猪被抓住了
        if human_num >= 3:
            Pig.count -= 1
    ...
    def move(self):
        ...
        if ((self.position[0] + 1 < self.field_height and field[self.position[0] + 1][self.position[1]] == 0) or \
            ...
            while True:
                direction = random.randint(0, 3)  # 0123，上下左右
                if direction == 0 and self.position[0] - 1 >= 0 and field[self.position[0] - 1][
                    self.position[1]] == 0:  # 如果向上有路可走
                    self.position[0] -= 1
                    break
                elif direction == 1 and self.position[0] + 1 < self.field_height and field[self.position[0] + 1][
                    self.position[1]] == 0:  # 如果向下有路可走
                    self.position[0] += 1
                    break
                elif direction == 2 and self.position[1] - 1 >= 0 and field[self.position[0]][
                    self.position[1] - 1] == 0:  # 如果向左有路可走
                    self.position[1] -= 1
                    break
                elif direction == 3 and self.position[1] + 1 < self.field_width and field[self.position[0]][
                    self.position[1] + 1] == 0:  # 如果向右有路可走
                    self.position[1] += 1
                    break
...
for i in range(20):
    for j in range(50):
        # 随机为每个人分配目标
        human_list.append(Human([20 + 2 * j, 10 + 2 * i], pig_list[random.randint(0, len(pig_list) - 1)],
                                (field_width, field_height)))

# 画出刚刚创建的猪和人类
for i in pig_list:
    # 画布，颜色，矩形对象（坐标，尺寸），圆角半径
    pygame.draw.rect(screen, pig_color, pygame.Rect((i.position[1] * size, i.position[0] * size), (size, size)),
                     border_radius=size // 2)
for i in human_list:
    pygame.draw.rect(screen, human_color, pygame.Rect((i.position[1] * size, i.position[0] * size), (size, size)),
                     border_radius=size // 2)
...
while True:
    ...
    frame += 1
    screen.blit(
        pygame.font.Font("SourceHanSansSC-Regular.otf", 24).render("模拟时间%d分钟" % (frame * 4), 1, (64, 255, 212)),
        (350, 2))
    ...
    # 检查猪抓完没有，并绘制猪数量的提示文字
    if not len(pig_list):
        screen.blit(pygame.font.Font("SourceHanSansSC-Regular.otf", 50).render(
            "任务完成，全部的猪都抓到啦，耗时%d分钟" % (frame * 4), 1, (61, 216, 255)), (200, 400))
        ...
        screen.blit(
            pygame.font.Font("SourceHanSansSC-Regular.otf", 24).render("还剩%d只猪" % Pig.count, 1, (255, 92, 201)),
            (580, 2))

    # 绘制是否开启目标线的提示文字
    if draw_target_line:
        screen.blit(
            pygame.font.Font("SourceHanSansSC-Regular.otf", 22).render("按下S键显示目标线：已开启", 1, (255, 255, 41)),
            (680, 750))
    else:
        screen.blit(
            pygame.font.Font("SourceHanSansSC-Regular.otf", 22).render("按下S键显示目标线：已隐藏", 1, (255, 255, 41)),
            (680, 750))
    ...
    for i in human_list:
        ...
        i.move()
        if draw_target_line:
            pygame.draw.aaline(screen, target_line_color,
                               (i.position[1] * size + size // 2, i.position[0] * size + size // 2),
                               (i.target.position[1] * size + size // 2, i.target.position[0] * size + size // 2), 1)
    ...
    screen.blit(pygame.font.Font("SourceHanSansSC-Regular.otf", 22).render("投个币吧(*^▽^*)", 1, (92, 255, 255)),
                (430, 750))
